...
def protein_weight(protein_str):
    return sum(mass_table[res] for res in protein_str)


if __name__ == "__main__":
    with open("in\prtm.txt", "r") as dataset:
        weight = protein_weight(dataset.readline().replace('\n', ""))
    print("%.3f" % weight)